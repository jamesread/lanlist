...
$sql = 'SELECT u.id, u.username, u.usernameSteam, u.discordUser, g.id AS groupId, g.title AS groupTitle, u.email, u.lastLogin, u.registered, o.id AS organizerId, o.title AS organizerTitle FROM users u JOIN groups g ON u.group = g.id LEFT JOIN organizers o ON u.organization = o.id WHERE u.id = :id GROUP BY u.id';
$stmt = $db->prepare($sql);
...
if (Session::hasPriv('VIEW_LOGS')) {
    $tpl->assign('userAuditLogs', lanlistFetchAuditLogsForUser($id));
}
